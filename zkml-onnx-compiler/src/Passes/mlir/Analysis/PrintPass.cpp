...
StringRef zk_ml_toolchain::PrintPass::getArgument() const {
    return "print-pass";
}
...
void zk_ml_toolchain::PrintPass::printOperation(Operation *op, std::vector<llvm::hash_code> &typeIds) {
    ...
    printIndent() << "visiting op: '" << op->getName() << "' with " << numOperands << " operands and " << numResults
                  << " results\n";
    ...
    if (opName == "arith.constant") {
        ...
        if (std::find(typeIds.begin(), typeIds.end(), hash) != typeIds.end()) {
            ...
            std::cout << *(std::find(typeIds.begin(), typeIds.end(), hash)) << std::endl;
        } else {
...
zk_ml_toolchain::PrintPass::IdentRAII::~IdentRAII() {
    --indent;
}

void zk_ml_toolchain::PrintPass::resetIndent() {
    indent = 0;
}

zk_ml_toolchain::PrintPass::IdentRAII zk_ml_toolchain::PrintPass::pushIndent() {
