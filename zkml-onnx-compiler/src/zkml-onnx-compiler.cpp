...
llvm::cl::opt<std::string>
    OutputFilename("i", llvm::cl::desc("Specify output filename"),
                   llvm::cl::value_desc("filename"),
                   llvm::cl::init(STDOUT_MARKER));

llvm::cl::opt<EmitLevel>
    EmitLevel(llvm::cl::desc("Which lowering level do you want?"),
...
llvm::cl::opt<bool> ZkMlDebugFlag("DEBUG",
                                  llvm::cl::desc("turns on debugging log"),
                                  llvm::cl::init(false));
...
int loadOnnxFile(StringRef inputFilename, mlir::MLIRContext &context,
...
lowerToLLVM(llvm::LLVMContext &llvmContext,
            mlir::OwningOpRef<mlir::ModuleOp> &mlirModule, int *error_code) {
...
void runZkMlPasses(std::unique_ptr<llvm::Module> &llvm_module,
...
void outputModule(mlir::OwningOpRef<mlir::ModuleOp> &module,
                  std::string &outputFilename, int64_t largeElementLimit = -1) {
...
std::unique_ptr<llvm::Module>
translateToLLVMIR(mlir::ModuleOp module, llvm::LLVMContext &llvm_context) {
...
int main(int argc, char **argv) {
  ...
  mlir::OwningOpRef<mlir::ModuleOp> module;
  std::string errorMessage;
  if (int rc = loadOnnxFile(llvm::StringRef(inputFilename), context, module,
                            &errorMessage)) {
  ...
  if (EmitMLIR || EmitLevel::ONNX == EmitLevel) {
    ...
    std::unique_ptr<llvm::Module> llvm_module =
        lowerToLLVM(llvmContext, module, &error_code);
    if (!llvm_module)
      return error_code;
    ...
    if (outputFilename.compare(STDOUT_MARKER) == 0) {
      ...
      std::error_code fileError;
      llvm::raw_fd_ostream fileStream(llvm::StringRef(outputFilename),
                                      fileError);
