...
void print_hex_byteblob(std::ostream &os, TIter iter_begin, TIter iter_end,
                        bool endl) {
  os << "0x" << std::hex;
...
void print_circuit(const circuit_proxy<ArithmetizationType> &circuit_proxy,
  ...
  using plonk_constraint_system = nil::marshalling::types::bundle<
      TTypeBase,
      std::tuple<
          nil::crypto3::marshalling::types::plonk_gates<
              TTypeBase, typename ConstraintSystemType::gates_container_type::
                             value_type>, // gates
          ...
          nil::crypto3::marshalling::types::plonk_lookup_gates<
              TTypeBase,
              typename ConstraintSystemType::lookup_gates_container_type::
                  value_type>, // lookup constraints
          // If we don't have lookup gates, we don't need lookup tables
          nil::crypto3::marshalling::types::plonk_lookup_tables<
              TTypeBase, typename ConstraintSystemType::lookup_tables_type::
                             value_type> // lookup tables
          >>;
  using AssignmentTableType = assignment_proxy<ArithmetizationType>;
  using variable_type = crypto3::zk::snark::plonk_variable<
  ...
  typename ConstraintSystemType::copy_constraints_container_type
      used_copy_constraints;
  const auto &used_copy_constraints_idx =
      circuit_proxy.get_used_copy_constraints();
    ...
    for (const auto &row : used_rows) {
      for (auto &constraint : used_copy_constraints) {
        ...
        if ((first_var.type == variable_type::column_type::witness ||
          ...
          constraint.first = variable_type(first_var.index, local_row,
                                           first_var.relative, first_var.type);
        }
        if ((second_var.type == variable_type::column_type::witness ||
          ...
          constraint.second =
              variable_type(second_var.index, local_row, second_var.relative,
                            second_var.type);
        }
      }
      local_row++;
  ...
  auto filled_val = plonk_constraint_system(std::make_tuple(
      nil::crypto3::marshalling::types::fill_plonk_gates<
          Endianness,
          typename ConstraintSystemType::gates_container_type::value_type>(
          used_gates),
      nil::crypto3::marshalling::types::fill_plonk_copy_constraints<
          Endianness, typename ConstraintSystemType::variable_type>(
          used_copy_constraints),
      nil::crypto3::marshalling::types::fill_plonk_lookup_gates<
          Endianness, typename ConstraintSystemType::
                          lookup_gates_container_type::value_type>(
          used_lookup_gates),
      nil::crypto3::marshalling::types::fill_plonk_lookup_tables<
          Endianness,
          typename ConstraintSystemType::lookup_tables_type::value_type>(
          used_lookup_tables)));
  ...
  nil::marshalling::status_type status =
      filled_val.write(write_iter, cv.size());
...
void fill_vector_value(std::vector<ValueType> &table_values,
                       const ContainerType &table_col, std::uint32_t padding) {
  std::copy(table_col.begin(), table_col.end(),
            std::back_inserter(table_values));
...
void print_assignment_table(
    const assignment_proxy<ArithmetizationType> &table_proxy,
    print_table_kind print_kind, std::uint32_t ComponentConstantColumns,
    std::uint32_t ComponentSelectorColumns, std::ostream &out = std::cout) {
  using AssignmentTableType = assignment_proxy<ArithmetizationType>;
  ...
  for (std::uint32_t i = 0; i < public_input_size; i++) {
    max_public_inputs_size = std::max(max_public_inputs_size,
                                      table_proxy.public_input_column_size(i));
  }

  if (print_kind == print_table_kind::PRIVATE) {
    total_columns = witness_size + shared_size + public_input_size +
                    constant_size + selector_size;
    std::uint32_t max_shared_size = 0;
    for (std::uint32_t i = 0; i < shared_size; i++) {
      max_shared_size =
          std::max(max_shared_size, table_proxy.shared_column_size(i));
    }
    for (const auto &i : lookup_constant_cols) {
      max_constant_size =
          std::max(max_constant_size, table_proxy.constant_column_size(i));
    }
    for (const auto &i : lookup_selector_cols) {
      max_selector_size =
          std::max(max_selector_size, table_proxy.selector_column_size(i));
    }
    usable_rows_amount = table_proxy.get_used_rows().size();
    usable_rows_amount =
        std::max({usable_rows_amount, max_shared_size, max_public_inputs_size,
                  max_constant_size, max_selector_size});
  } else { // FULL
    ...
    for (std::uint32_t i = 0; i < witness_size; i++) {
      max_witness_size =
          std::max(max_witness_size, table_proxy.witness_column_size(i));
    }
    ...
    usable_rows_amount = std::max({max_witness_size, max_public_inputs_size,
                                   max_constant_size, max_selector_size});
  }

  std::uint32_t padded_rows_amount =
      std::pow(2, std::ceil(std::log2(usable_rows_amount)));
  ...
  std::vector<typename AssignmentTableType::field_type::value_type>
      table_values;
  if (print_kind == print_table_kind::FULL) {
    for (std::uint32_t i = 0; i < witness_size; i++) {
      fill_vector_value<typename AssignmentTableType::field_type::value_type,
                        column_type>(table_values, table_proxy.witness(i),
                                     padded_rows_amount);
    }
    for (std::uint32_t i = 0; i < public_input_size; i++) {
      fill_vector_value<typename AssignmentTableType::field_type::value_type,
                        column_type>(table_values, table_proxy.public_input(i),
                                     padded_rows_amount);
    }
    for (std::uint32_t i = 0; i < constant_size; i++) {
      fill_vector_value<typename AssignmentTableType::field_type::value_type,
                        column_type>(table_values, table_proxy.constant(i),
                                     padded_rows_amount);
    }
    for (std::uint32_t i = 0; i < selector_size; i++) {
      fill_vector_value<typename AssignmentTableType::field_type::value_type,
                        column_type>(table_values, table_proxy.selector(i),
                                     padded_rows_amount);
    }
  } else {
    ...
    // witness
    for (std::size_t i = 0;
         i < AssignmentTableType::arithmetization_params::witness_columns;
         i++) {
    ...
    for (std::uint32_t i = 0; i < shared_size; i++) {
      fill_vector_value<typename AssignmentTableType::field_type::value_type,
                        column_type>(table_values, table_proxy.shared(i),
                                     padded_rows_amount);
    }
    // constant
  ...
  auto filled_val = plonk_assignment_table(std::make_tuple(
      ...
      nil::crypto3::marshalling::types::fill_field_element_vector<
          typename AssignmentTableType::field_type::value_type, Endianness>(
          table_values)));
...
int curve_dependent_main(std::string bytecode_file_name,
                         ...
                         std::string circuit_file_name, long stack_size,
                         bool check_validity,
  ...
  constexpr std::size_t ConstantColumns =
      ComponentConstantColumns + LookupConstantColumns;
  constexpr std::size_t SelectorColumns =
      ComponentSelectorColumns + LookupSelectorConstantColumns;
  ...
  using ConstraintSystemProxyType =
      zk::snark::plonk_table<BlueprintFieldType, ArithmetizationParams,
                             zk::snark::plonk_column<BlueprintFieldType>>;
  ...
  if (!input_file.is_open()) {
    std::cerr << "Could not open the file - '" << public_input_file_name << "'"
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  nil::blueprint::parser<BlueprintFieldType, ArithmetizationParams,
                         PrintCircuitOutput>
      parser_instance(stack_size, log_level, max_num_provers, policy);
  ...
  if (!parser_instance.evaluate(std::move(module),
                                input_json_value.as_array())) {
    return 1;
  }
  // end of changes

  ASSERT_MSG(!parser_instance.assignments.empty() &&
                 !parser_instance.circuits.empty(),
  ...
  if (parser_instance.circuits[0].get_reserved_tables().size() > 0) {
    ...
    std::iota(lookup_columns_indices.begin(), lookup_columns_indices.end(),
    ...
    auto usable_rows_amount = zk::snark::pack_lookup_tables_horizontal(
        ...
        parser_instance.circuits[0].get(), parser_instance.assignments[0].get(),
        lookup_columns_indices, ComponentSelectorColumns,
        parser_instance.assignments[0].allocated_rows(), max_usable_rows);
  }
  ...
  if (parser_instance.assignments.size() == 1) {
    ...
    if (!otable) {
      std::cout << "Something wrong with output " << assignment_table_file_name
                << std::endl;
      return 1;
    }
    ...
    print_assignment_table<nil::marshalling::option::big_endian,
        ...
        parser_instance.assignments[0], print_table_kind::FULL,
        ComponentConstantColumns, ComponentSelectorColumns, otable);
    ...
    for (const auto &it : parser_instance.assignments) {
      ...
      if (!otable) {
        std::cout << "Something wrong with output "
                  << assignment_table_file_name + std::to_string(it.get_id())
                  << std::endl;
        return 1;
      }
      ...
      print_assignment_table<nil::marshalling::option::big_endian,
                             ArithmetizationType, BlueprintFieldType>(
          it, print_table_kind::PRIVATE, ComponentConstantColumns,
  ...
  for (auto &it : parser_instance.circuits) {
    ...
    std::string file_name =
        parser_instance.circuits.size() > 1
            ? circuit_file_name + std::to_string(it.get_id())
            : circuit_file_name;
    ...
    ASSERT_MSG(assignment_it != parser_instance.assignments.end(),
               "Not found assignment for circuit");
    print_circuit<nil::marshalling::option::big_endian, ArithmetizationType,
  ...
  if (check_validity) {
    ASSERT_MSG(
        nil::blueprint::is_satisfied(parser_instance.circuits[0].get(),
    ...
    for (auto &it : parser_instance.circuits) {
      ...
      ASSERT_MSG(is_accessible, ("The circuit is not satisfied on prover " +
...
#define str(s) #s
  ...
  if (vm.count("bytecode")) {
    ...
    std::cerr
        << "Invalid command line argument - bytecode file name is not specified"
        << std::endl;
    std::cout << options_desc << std::endl;
    return 1;
  }
  ...
  if (vm.count("circuit")) {
    ...
    std::cerr
        << "Invalid command line argument - circuit file name is not specified"
        << std::endl;
    std::cout << options_desc << std::endl;
    return 1;
  }
  ...
  if (curve_options.find(elliptic_curve) == curve_options.end()) {
    std::cerr
        << "Invalid command line argument -e (Native elliptic curve type): "
  ...
  if (vm.count("policy")) {
    ...
    if (policy != "default") {
      std::cerr << "Invalid command line argument - policy. " << policy
  ...
  if (vm.count("max-num-provers")) {
    ...
    if (max_num_provers < 1) {
      std::cerr << "Invalid command line argument - max-num-provers. "
                << max_num_provers << " is wrong value." << std::endl;
      std::cout << options_desc << std::endl;
      return 1;
    }
  }
  ...
  if (log_options.find(log_level) == log_options.end()) {
    std::cerr << "Invalid command line argument -l (log level): " << log_level
              << std::endl;
    std::cout << options_desc << std::endl;
    return 1;
  }
  ...
  case 0: {
    if (vm.count("print_circuit_output")) {
      return curve_dependent_main<typename algebra::curves::pallas, true>(
          bytecode_file_name, public_input_file_name,
          assignment_table_file_name, circuit_file_name, stack_size,
          vm.count("check"), log_options[log_level], policy, max_num_provers);
    } else {
