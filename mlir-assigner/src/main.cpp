...
void print_circuit(const ConstraintSystemType &circuit,
                   std::ostream &out = std::cout) {

  using TTypeBase = nil::marshalling::field_type<Endianness>;
  auto filled_val =
      nil::crypto3::marshalling::types::fill_plonk_constraint_system<
          Endianness, ConstraintSystemType>(circuit);
...
void print_assignment_table(std::size_t usable_rows,
                            const AssignmentTableType &table,
                            std::ostream &out = std::cout) {
  using TTypeBase = nil::marshalling::field_type<Endianness>;
  auto filled_val = nil::crypto3::marshalling::types::fill_assignment_table<
      Endianness, AssignmentTableType>(usable_rows, table);
...
int curve_dependent_main(std::string bytecode_file_name,
                         ...
                         bool check_validity, bool verbose,
  ...
  //    nil::blueprint::profiling_assignment_table(parser_instance.assignmnt,
  //    desc.usable_rows_amount, otable);
  otable.close();

  std::ofstream ocircuit;
  ocircuit.open(circuit_file_name);
  if (!ocircuit) {
    std::cout << "Something wrong with output " << circuit_file_name
              << std::endl;
    return 1;
  }
  print_circuit<nil::marshalling::option::big_endian, ConstraintSystemType>(
      parser_instance.bp, ocircuit);
  ocircuit.close();

  if (check_validity) {
    bool is_satisfied = nil::blueprint::is_satisfied(parser_instance.bp,
                                                     parser_instance.assignmnt);
  ...
  case 0: {
    if (vm.count("print_circuit_output")) {
      return curve_dependent_main<typename algebra::curves::pallas, true>(
          ...
          vm.count("check"), vm.count("verbose"), policy);
    } else {
